...
void move_player_and_view_horizontal(rpg_t *rpg, sfImage *image, bool move)
{
    sfColor color;

    if (rpg->all_events.right) {
        color = get_color_from_player(image, (sfVector2f){rpg->
        spritesheet[S_PLAYER].pos.x + 2, rpg->spritesheet[S_PLAYER].pos.y});
        if (rpg->spritesheet[S_PLAYER].pos.x > 385 &&
        rpg->begin.view.center.x < 1535 && color.r != 255 && move)
    ...
    if (rpg->all_events.left) {
        ...
        rpg->begin.view.center.x > 385 && color.r != 255 && move)
            rpg->begin.view.center.x -= 2;
        if (color.r != 255)
            rpg->spritesheet[S_PLAYER].pos.x -= 2;
    }
...
void move_player_and_view_vertical(rpg_t *rpg, sfImage *image, bool move)
{
    sfColor color;
    ...
    if (rpg->all_events.up) {
        ...
        rpg->begin.view.center.y > -203 && color.r != 255 && move)
    ...
    if (rpg->all_events.down) {
        ...
        rpg->begin.view.center.y < 1282 && color.r != 255 && move)
            rpg->begin.view.center.y += 2;
        if (color.r != 255)
            rpg->spritesheet[S_PLAYER].pos.y += 2;
    }
...
void move_player_and_view(rpg_t *rpg)
{
    if (rpg->screen[GAME].active) {
        move_player_and_view_horizontal(rpg, rpg->map_colors, true);
        move_player_and_view_vertical(rpg, rpg->map_colors, true);
    } else {
        move_player_and_view_horizontal(rpg, rpg->map_colors, false);
        move_player_and_view_vertical(rpg, rpg->map_colors, false);
    }
